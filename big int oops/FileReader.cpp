...
public:
	...
	//Constructors for big int type
	BigInt(unsigned long long nr)//for long int iif nothing then parameterized constructor
    {
	do{
	...
	BigInt(string & s)//this is constructor for input string
    {
	 digits = "";
	 int n = s.size();
	 for (int i = n - 1; i >= 0;i--){
	...
	BigInt(BigInt & a)//normal if bigint type
	...
	//Helper Functions//declaration of friend function 
...
BigInt &BigInt::operator++(){// return type class name::name of the function
	int i, n = digits.size();
